...
ChiliBook = { //implied global
	...
	, replaceSpace:       "&#160;"                   // use an empty string for not Replace
	, replaceTab:         "&#160;&#160;&#160;&#160;" // use an empty string for not Replace
	, replaceNewLine:     "&#160;<br/>"              // use an empty string for not Replace
		...
		function chef( matched ) {
			...
			while( step = steps[ i++ ] ) {
				...
				if( aux[ j ] ) {
					...
					perfect += filter( unmatched ) + replacement; // use perfect for all the Replace
					return replacement;
				} 
				else {
					j+= step.length;
